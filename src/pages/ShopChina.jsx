...
export default function ShopChina() {
  ...
  const { data: allProducts = [], isLoading } = useQuery({
    ...
    queryFn: () => filterProducts({ type: 'china' }, '-created_date'),
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-sky-100 dark:bg-sky-500/20 flex items-center justify-center">
              <ShoppingBag className="w-5 h-5 text-sky-600 dark:text-sky-300" />
            </div>
            <h1 className="text-3xl md:text-4xl font-bold text-foreground">Товары из Китая</h1>
          </div>
          <p className="text-muted-foreground">
            {filteredProducts.length} товаров — проверенные находки с маркетплейсов
          </p>
        </div>
